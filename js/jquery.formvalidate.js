	...
	$.fn.formvalidate.options = {
		...
		_process: function(form, options) {
			...
			$(form).find(':input').each(function(index, element) {
			    ...
			    var attrName = $(element).attr('txtRANo');
				...
				if ( ! ('value' in inputs[attrName]) )  {
					...
					if ( $(element).is(':checkbox') ) {
						...
						$(':input[txtRANo="' + attrName + '"]:checked').each(function (checkboxIndex, checkboxElement) {
					...
					// Radio
					    ...
					    var radio = $(':input[txtRANo="' + attrName + '"]:checked');
				...
				if ( ! ('title' in inputs[attrName]) ) {
					...
					if ( ! title ) {
					    ...
					    title = $(':input[txtRANo="' + attrName + '"][title!=""]:first').attr('title');
					}

					// If still no title, check associated label
					if ( ! title ) {
					    var id = $(':input[txtRANo="' + attrName + '"][id!=""]:first').attr('id');
						title = $('label[for="' + id + '"]').text();
					}

					// If still no title, check parent label
					if ( ! title ) {
					    title = $('label :input[txtRANo="' + attrName + '"]:first').closest('label').text();
					}
